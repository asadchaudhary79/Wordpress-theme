 ...
 */
 ...
 (function() {
	...
	function createDropdownToggle() {
		...
		var icon = new DOMParser().parseFromString( drowanScreenReaderText.icon, 'text/html' ).body.firstElementChild;
		dropdownToggle.appendChild( icon);

		// Add screenreader text.
		var screenReaderText = document.createElement( 'span' );
		screenReaderText.classList.add( 'screen-reader-text' );
		screenReaderText.textContent = drowanScreenReaderText.expand;
	...
	function initNavigation( containerClass, naviClass ) {
		...
		(function() {
			...
			navigation.querySelectorAll( '.current-menu-ancestor > button' ).forEach( function( activeToggle ) {
				...
				activeToggle.querySelector( '.screen-reader-text' ).textContent = drowanScreenReaderText.collapse;
			});
			...
			navigation.querySelectorAll( '.dropdown-toggle' ).forEach( function( dropdownItem ) {
				dropdownItem.addEventListener( 'click', function() {
					...
					dropdownItem.querySelector( '.screen-reader-text' ).textContent = dropdownItem.classList.contains( 'toggled-on' ) ? drowanScreenReaderText.collapse : drowanScreenReaderText.expand;
