 ...
 */
function drowan_theme_addons_setup()
{

	// Add theme support for drowan Pro plugin.
	add_theme_support('drowan-pro');

	// Add theme support for WPCodeX Plugins.
	add_theme_support('WPCodeX-breadcrumbs');

	// Add theme support for Widget Bundle.
	add_theme_support('WPCodeX-widget-bundle', array(
		'thumbnail_size' => array(100, 80),
		'svg_icons'      => true,
	));

	// Add theme support for Related Posts.
	add_theme_support('WPCodeX-related-posts', array(
	...
	add_theme_support('infinite-scroll', array(
		...
		'wrapper'        => false,
		'render'         => 'drowan_infinite_scroll_render',
		'posts_per_page' => 6,
	));
...
add_action('after_setup_theme', 'drowan_theme_addons_setup');


/**
 * Custom render function for Infinite Scroll.
 */
function drowan_infinite_scroll_render()
{
	while (have_posts()) {
		the_post();
 ...
 */
function drowan_wrapper_start()
{
	echo '<section id="primary" class="content-area">';
	echo '<main id="main" class="site-main" role="main">';
}
remove_action('woocommerce_before_main_content', 'woocommerce_output_content_wrapper', 10);
add_action('woocommerce_before_main_content', 'drowan_wrapper_start', 10);


/**
 * Set wrapper end for wooCommerce
 */
function drowan_wrapper_end()
{
	echo '</main><!-- #main -->';
	echo '</section><!-- #primary -->';
}
remove_action('woocommerce_after_main_content', 'woocommerce_output_content_wrapper_end', 10);
add_action('woocommerce_after_main_content', 'drowan_wrapper_end', 10);


/**
 * Checks if AMP page is rendered.
 */
function drowan_is_amp()
{
 ...
 */
function drowan_amp_menu_toggle()
{
	if (drowan_is_amp()) {
 ...
 */
function drowan_amp_menu_is_toggled()
{
	if (drowan_is_amp()) {
 ...
 */
function drowan_amp_menu_dropdown_toggles($item_output, $item, $depth, $args)
{
	...
	*/
	...
	// Add SVG icon.
	$dropdown_button .= $expanded ? drowan_get_svg('collapse') : drowan_get_svg('expand');
	...
	$dropdown_button .= sprintf(
		...
		esc_attr(sprintf("$expanded_state_id ? %s : %s", wp_json_encode(esc_html__('Collapse child menu', 'drowan')), wp_json_encode(esc_html__('Expand child menu', 'drowan')))),
		esc_html($expanded ? esc_html__('Collapse child menu', 'drowan') : esc_html__('Expand child menu', 'drowan'))
	);
...
add_filter('walker_nav_menu_start_el', 'drowan_amp_menu_dropdown_toggles', 10, 4);
