 ...
 */
function drowan_get_svg($icon = null)
{
 ...
 */
function drowan_dropdown_icon_to_menu_link($title, $item, $args, $depth)
{
	if ('primary' === $args->theme_location || 'secondary' === $args->theme_location) {
		foreach ($item->classes as $value) {
			if ('menu-item-has-children' === $value || 'page_item_has_children' === $value) {
				$title = $title . drowan_get_svg('expand');
			}
		}
	}
...
add_filter('nav_menu_item_title', 'drowan_dropdown_icon_to_menu_link', 10, 4);
 ...
 * @return string $svg SVG markup.
 */
function drowan_get_social_svg($icon = null)
{
	// Return early if no icon was defined.
	if (empty($icon)) {
		return;
 ...
 */
function drowan_social_icons_menu($item_output, $item, $depth, $args)
{
	...
	$social_icons = drowan_supported_social_icons();
	...
	// Get SVG.
	$svg = apply_filters('drowan_get_social_svg', drowan_get_social_svg($icon), $item_output);
...
add_filter('walker_nav_menu_start_el', 'drowan_social_icons_menu', 10, 4);
 ...
 */
function drowan_supported_social_icons()
{
	...
	return apply_filters('drowan_supported_social_icons', $supported_social_icons);
}
